...
namespace {
/// GBNF grammar for the suggest-tags contract: {"summary": <string>, "tags": [1..3 strings]}.
/// Constrained decoding makes syntactically invalid JSON impossible; the
/// AiJsonSanitizer layer then only enforces semantics (dedup, reject list, caps).
constexpr const char kSuggestTagsGrammar[] = R"GBNF(
root ::= "{" ws "\"summary\"" ws ":" ws string ws "," ws "\"tags\"" ws ":" ws tags ws "}"
tags ::= "[" ws string (ws "," ws string)? (ws "," ws string)? ws "]"
string ::= "\"" char{1,300} "\""
                                              ...
                                              const char *grammarGbnf)
{
  ...
  if (grammarGbnf) {
    struct llama_sampler *gs = llama_sampler_init_grammar(vocab, grammarGbnf, "root");
    if (gs)
      llama_sampler_chain_add(smpl, gs);
    else
      qWarning() << "[AI] GBNF grammar failed to parse; falling back to unconstrained decoding.";
  }
                                     ...
                                     const std::string &correctionExamples)
{
  ...
  {
    QMutexLocker<QRecursiveMutex> locker(&m_mutex);
    m_lastRawSuggestTags = QString::fromStdString(raw);
  }
  return SfAiJson::ensureSuggestTagsJson(raw, rejectedTagsCsv);
}

QString LlamaEngine::lastRawSuggestTagsOutput() const
{
  QMutexLocker<QRecursiveMutex> locker(&m_mutex);
  return m_lastRawSuggestTags;
}
                                         ...
                                         const std::string &correctionExamples)
{
  ...
  if (pdfMetadataOnly) {
    ...
    return generateResponseImpl(prompt, kMaxNewTokensSuggestTagsJson, kSuggestTagsGrammar);
  }
  ...
  if (!correctionExamples.empty()) {
    prompt += en ? "\nPast user corrections on similar files (avoid repeating these mistakes):\n" + correctionExamples
                 : "\n使用者過去對相似檔案的糾錯紀錄（參考以避免類似誤判）：\n" + correctionExamples;
  }
